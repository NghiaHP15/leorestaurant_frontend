...
export default function EditSupplierIngredient({
  valueSupplier,
  mutation,
  show,
  setShow,
  ...
  useEffect(() => {
    if (isSuccess && data?.data) {
      toast.current.show({
        severity: "success",
        summary: "Thành công",
        detail: `Sửa dữ liệu ${valueSupplier.name} thành công`,
        icon: (
          <FontAwesomeIcon
            icon={faCircleCheck}
  ...
  useEffect(() => {
    const getListStaff = async () => {
      const result = await SupplierService.getDetail(valueSupplier._id);
      setUploadedImageUrl(result.data.image);
      setDataSupplier(result.data);
    };
    getListStaff();
  }, [valueSupplier]);
  ...
  const handleUpload = async (event) => {
    ...
    if (totalSize < 2000000) {
      ...
      console.log(image);
      _data.image = image.data.imagesPath;
          ...
          <div className="grid">
            <div className="col-12">
              <h2 className="m-0 px-2 capitalize">
                Sửa nhà cung cấp {valueSupplier.name}
              </h2>
            </div>
            <div className="col-8">
              <div className="col-12">
