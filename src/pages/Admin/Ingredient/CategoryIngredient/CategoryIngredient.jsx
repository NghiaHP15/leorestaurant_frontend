...
import CreateCategoryIngredient from "./CreateCatagoryIngredient";
...
function ListCategoryIngredient() {
  ...
  const mutationCreate = userMutationHook((data) =>
    CategoryIngridient.createCategoryIngredient(data)
  );
  const mutationEdit = userMutationHook((data) =>
    CategoryIngridient.editCategoryIngredient(data, data._id)
  );
  const mutationDelete = userMutationHook((data) =>
    CategoryIngridient.deleteCategoryIngredient(data)
  );
  ...
  useEffect(() => {
    const getListData = async () => {
      setLoading(true);
      const data = await CategoryIngridient.listCategoryIngredient();
      setLoading(false);
        ...
        {!listData && (
          <div className="absolute top-0 bottom-0 right-0 left-0">
            <LoadingTable
              count={2}
              selection
              data={["Tên danh mục", "Mô Tả", "Hành Động"]}
            />
          </div>
        )}
      </div>
...
export default ListCategoryIngredient;
