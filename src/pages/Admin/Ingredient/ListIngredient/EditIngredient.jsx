...
export default function EditIngredient({ show, setShow, mutation, dataRow }) {
  ...
  useEffect(() => {
    if (isSuccess && data?.data) {
      toast.current.show({
        severity: "success",
        summary: "Thành công",
        detail: `Sửa dữ liệu ${dataRow.name} thành công`,
        icon: (
          <FontAwesomeIcon
            icon={faCircleCheck}
  ...
  useEffect(() => {
    const fechData = async () => {
      const [categoryResult, supplierResult, ingredientResult] =
        await Promise.all([
          CategoryIngridient.listCategoryIngredient(),
          SupplierService.listSupplier(),
          IngredientService.detailIngredient(dataRow._id),
        ]);
      setCategory(categoryResult.data);
      setSupplier(supplierResult.data);
      setDataIngredient(ingredientResult.data);
      setUploadedImageUrl(ingredientResult.data.image);
    };
    fechData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dataRow]);
        ...
        <div className="overflow-hidden">
          ...
          <div className="grid">
            <div className="col-12">
              <h2 className="m-0 px-2 capitalize">
                Sửa nguyên liệu {dataRow.name}
              </h2>
            </div>
            <div className="col-8">
              <div className="col-12">
