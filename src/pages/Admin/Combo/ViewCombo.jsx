...
export default function CreateCombo() {
  ...
  const mutation = userMutationHook((data) =>
    ComboService.editCombo(data._id, data)
  );
  ...
  useEffect(() => {
    if (location.state?.id) {
      ...
      const fetchData = async () => {
        setLoading(true);
        const [menuResult] = await Promise.all([ComboService.detailCombo(id)]);
        setDataCombo(menuResult.data);
  ...
  useEffect(() => {
    if (isSuccess && data?.data) {
      toast.current.show({
        severity: "success",
        summary: "Thành công",
        detail: `Sửa combo ${data.data.name} thành công`,
        icon: (
          <FontAwesomeIcon
            icon={faCircleCheck}
