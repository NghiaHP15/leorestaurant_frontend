...
function ListCategoryIngredient() {
  ...
  const mutationEdit = userMutationHook((data) =>
    CategoryFood.editCategoryFood(data, data._id)
  );
  const mutationDelete = userMutationHook((data) =>
    CategoryFood.deleteCategoryFood(data)
  );
  ...
  useEffect(() => {
    const getListData = async () => {
      setLoading(true);
      const data = await CategoryFood.listCategoryFood();
      setLoading(false);
  ...
  const deleteSelected = () => {
    ...
    dataDelete?.deletedCount &&
      toast.current.show({
        severity: "success",
        summary: "Thành công",
        detail: `Xóa dữ liệu thành công ${dataDelete.deletedCount} danh mục món`,
        icon: (
          <FontAwesomeIcon
            icon={faCircleCheck}
            className="text-green-500 h-2rem"
  ...
  return (
    <div className="p-4 mt-4 shadow-2 bg-white border-round-xl">
      ...
      <OverlayPanel ref={create}>
        <CreateCategoryDish createRef={create} mutation={mutationCreate} />
      </OverlayPanel>
    </div>
  );
}
