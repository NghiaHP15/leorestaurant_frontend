...
import CreateCategoryTable from "./";
...
function CategoryTable() {
  ...
  const mutationCreate = userMutationHook((data) =>
    CategoryTableService.createCateroryTable(data)
  );

  const mutationDelete = userMutationHook((data) =>
    CategoryTableService.deleteCateroryTable(data)
  );
  ...
  useEffect(() => {
    const getListData = async () => {
      setLoading(true);
      const data = await CategoryTableService.listCateroryTable();
      setLoading(false);
    ...
    <div className="grid mt-4 ">
      <div className="col-3">
        <div className="flex flex-column font-family">
          <span className="font-medium ">Danh sách danh mục bàn</span>
          <span className="text-color-secondary mt-2">
            Quản lý các loại bàn của bạn trong nhà hàng
          </span>
          <Link
            to={config.router.settingTable}
      ...
      <div className="col-9">
        <div className="p-4 shadow-2 bg-white border-round-xl">
          ...
          <OverlayPanel ref={create}>
            <CreateCategoryTable createRef={create} mutation={mutationCreate} />
          </OverlayPanel>
        </div>
      </div>
    </div>
