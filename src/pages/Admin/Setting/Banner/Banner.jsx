...
import CreateCategoryBanner from "./CreateCategoryBanner";
import { OverlayPanel } from "primereact/overlaypanel";

function CatrgoryBanner() {
  ...
  const mutationCreate = userMutationHook((data) =>
    CategoryBanner.createCategoryBanner(data)
  );

  const mutationDelete = userMutationHook((data) =>
    CategoryBanner.deleteCategoryBanner(data)
  );
  ...
  useEffect(() => {
    const getListData = async () => {
      setLoading(true);
      const data = await CategoryBanner.listCategoryBanner();
      setLoading(false);
      setListData(data.data);
    };
    getListData();
  }, [dataCreate, dataDelete]);
    ...
    <div className="grid mt-4 ">
      <div className="col-3">
        <div className="flex flex-column font-family">
          <span className="font-medium ">Danh sách danh mục banner</span>
          <span className="text-color-secondary mt-2">
            Quản lý các loại hình ảnh quảng cáo
          </span>
        </div>
      </div>
      <div className="col-9">
