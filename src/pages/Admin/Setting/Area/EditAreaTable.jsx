...
export default function EditArea({ show, setShow, mutation, dataRow }) {
  ...
  useEffect(() => {
    const fechData = async () => {
      const [tableResult, categoryTable, areaResult] = await Promise.all([
        TableService.listTableFilter(),
        CategoryTableService.listCateroryTable(),
        AreaService.getDetail(dataRow._id),
      ]);
      ...
      setDataArea(areaResult.data);
      setSelectedValue(areaResult.data.table);
    };
    fechData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [show]);
