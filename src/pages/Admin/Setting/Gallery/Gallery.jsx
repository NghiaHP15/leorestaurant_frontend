...
function Gallery() {
  ...
  const mutationCreate = userMutationHook((data) =>
    CategoryGallery.createCategoryGallery(data)
  );

  const mutationDelete = userMutationHook((data) =>
    CategoryGallery.deleteCategoryGallery(data)
  );
  ...
  useEffect(() => {
    const getListData = async () => {
      setLoading(true);
      const data = await CategoryGallery.listCategoryGallery();
      setLoading(false);
    ...
    <div className="grid mt-4 ">
      <div className="col-3">
        <div className="flex flex-column font-family">
          <span className="font-medium ">Danh sách danh mục bộ sưu tập</span>
          <span className="text-color-secondary mt-2">
            Quản lý các loại hình ảnh
          </span>
        </div>
      </div>
      <div className="col-9">
        <div className="p-4 shadow-2 bg-white border-round-xl">
          ...
          <OverlayPanel ref={create}>
            <CreateGallery createRef={create} mutation={mutationCreate} />
          </OverlayPanel>
        </div>
      </div>
    </div>
