...
function Banner() {
  ...
  const [filters, setFilters] = useState({
    "categoryBanner._id": { value: null, matchMode: FilterMatchMode.EQUALS },
  });
  ...
  const mutationEdit = userMutationHook((data) =>
    BannerService.editBanner(data._id, data)
  );
  const mutationDelete = userMutationHook((data) =>
    BannerService.deleteBanner(data)
  );
  ...
  useEffect(() => {
    const getListMenu = async () => {
      setLoading(true);
      const resultBanner = await BannerService.listBanner();
      const resultCategory = await CategoryBanner.listCategoryBanner();
      setLoading(false);
      setListData(resultBanner.data);
      const dataCategory = [
        { _id: "", name: "Mặc định" },
        ...resultCategory.data,
      ];
      setCategoryBanner(dataCategory);
    };
    getListMenu();
  ...
  const onCategoryFilterChange = (e) => {
    ...
    _filters["categoryBanner._id"].value = value;
    setFilters(_filters);
  ...
  const categoryBodyTemplate = (rowData) => {
    return <span>{rowData.categoryBanner?.name || "Không có"}</span>;
  };
